...
|------|-----------|-------------------|
| **Конфигурация** | superset-secrets | Хранит учетные данные: пароли PostgreSQL и Redis, секретный ключ Superset |
| | superset-sa | ServiceAccount для предоставления прав доступа Superset внутри кластера |
| **Слой данных** | hostPath (/tmp/postgres-data) | Хранилище данных PostgreSQL на хостовой машине (3Gi), обеспечивает персистентность |
| | PostgreSQL Pod | Контейнер с PostgreSQL 16, содержит базу данных superset и таблицу sales |
| | postgres-service | ClusterIP-сервис для доступа к PostgreSQL из других подов кластера |
| **Слой кэширования** | Redis Pod | Контейнер с Redis 7-alpine, используется для кэширования запросов и хранения сессий |
| | redis-service | ClusterIP-сервис для доступа к Redis из Superset |
| **Слой аналитики** | Superset Pod | Основной контейнер с Apache Superset 6.0.0 (кастомный образ my-superset:v1), порт 8088 |
| | superset-init | Init-контейнер, выполняет миграции базы данных, создает администратора и инициализирует Superset |
| | superset-service | NodePort-сервис (30088) для доступа к Superset из браузера |
| **Загрузка данных** | Data Generator Job | Однократная задача (Job), генерирующая 1000+ тестовых записей и загружающая их в таблицу sales |
| **Пользователь** | Аналитик | Конечный пользователь, подключается к Superset через браузер для создания дашбордов и анализа данных |
